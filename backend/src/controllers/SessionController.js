...
module.exports = {
    async create(request, response){
        ...
        const airline_email = await connection('airlines').where('email', email).select('name').first();
        const airline_password = await connection('airlines').where('password', password).select('name').first();
